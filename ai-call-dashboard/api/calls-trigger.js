...
export default async (req, context) => {
  ...
  try {
    ...
    context.waitUntil(
      fetch(process.env.WORKFLOW_URL ?? "https://cloud.xpectrum.dev/v1/workflows/run", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${process.env.WORKFLOW_API_KEY ?? "xpectrum_fYnCTfIIpOhvQO8a2jELLapm"}`,
